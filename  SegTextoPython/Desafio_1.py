...
def frase_mais_longa():
    texto = entrada.get("1.0", "end-1c")
    frases = tokenizer_frases.tokenize(texto)
    
    # Encontrar a frase com o maior número de palavras
    max_palavras = 0
    frase_longa = ""
    
    for frase in frases:
        palavras = tokenizador_palavras.tokenize(frase)
    ...
    saida.delete("1.0", "end")
    saida.insert("end", f"Frase mais longa: \n{frase_longa}\n")
    saida.insert("end", f"Número de palavras: {max_palavras}")
...
tk.Button(frame_botoes, text=" Frase Mais Longa", command=frase_mais_longa, **button_style).pack(side="left", padx=10)
